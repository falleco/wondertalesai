import { CheckIcon } from "@web/components/icons";
import { cn } from "@web/lib/utils";
import Link from "next/link";
// import { updateSubscriptionPlan } from "@/actions/payments";
import { useRouter } from "next/navigation";
import type { PropsWithChildren } from "react";
import { toast } from "sonner";
import type { TBILLING_PLAN } from "./data";
import GlowGradient from "./glow";
import { handlePurchase } from "./handle-purchase";
        ...
        <div className="p-8">
          ...
          {plan.name.includes("Enterprise") ? (
            ...
            <button
              type="button"
              onClick={async () => {
                const priceId = plan.pricing[billingPeriod].stripeId ?? "";

                if (activeSubscriptionId) {
                  // await updateSubscriptionPlan(priceId, activeSubscriptionId);
                  toast.success("Subscription plan change successful");
                  router.push("/billing");
                } else {
                  await handlePurchase(priceId);
                }
              }}
              className={cn(
