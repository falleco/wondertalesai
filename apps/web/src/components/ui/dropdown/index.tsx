...
import { cn, getScrollBarWidth } from "@web/lib/utils";
import type { SetStateActionType } from "@web/types/set-state-action-type";
...
export function Dropdown({ children, isOpen, setIsOpen }: DropdownProps) {
  ...
  return (
    ...
    >
      {/* biome-ignore lint/a11y/noStaticElementInteractions: This div is a container for dropdown keyboard navigation */}
    ...
    if (isOpen) {
      ...
      if (
        ...
        contentRef.current?.style.setProperty(
          "transform",
          `translate(0, -${
            height + contentRef.current?.offsetHeight + sideOffset * 2
          }px)`,
        );
      } else {
      ...
      if (align === "center") {
        leftPosition = left - Number(triggerRef?.current?.clientWidth ?? 0) / 2;
      }

      contentRef.current?.style.setProperty("left", `${leftPosition}px`);
    }
  }, [isOpen, align, dropdownRef, contentRef, sideOffset, triggerRef]);
